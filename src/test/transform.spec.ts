...
import { end, start, transform } from "../main/transform";
  ...
  const check = (
    ...
    it(testName, function () {
      ...
      const rig = startLines
        .concat(anchorLines ? anchorLines.slice(0, -2) : [])
        .concat(endLines)
        .concat("")
        .concat(mdLines);
      assert.isTrue(res.transformed, "transforms it");
      assert.deepEqual(rig, data, "generates correct anchors");
    });
  };
